...
def main():
    ...
    data = pd.read_csv(get_data_path('USD_KRW'), thousands=',')
    test_x, test_y = DataGenerator.make_features(data)

    ###################################################################################################################
    # inspect test data
    assert test_y == get_test_dollar_price(start_date, end_date).tolist(), 'your test data is wrong!'
    ###################################################################################################################
